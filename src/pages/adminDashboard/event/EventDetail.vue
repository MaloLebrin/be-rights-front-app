        ...
        <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8 xl:flex xl:items-center xl:justify-between">
          <div class="flex-1 min-w-0">
            <div class="flex items-center space-x-10">
              <h1 class="mt-2 text-2xl font-bold leading-7 text-gray-900 sm:text-3xl sm:truncate">{{ event.name }}</h1>
              <EventStatusTag :status="event.status" />
            </div>
            <div class="flex flex-col mt-1 sm:flex-row sm:flex-wrap sm:mt-0 sm:space-x-8">
              <div class="flex items-center mt-2 text-sm text-gray-500">
                <LocationMarkerIconOutline class="flex-shrink-0 mr-1.5 h-5 w-5 text-gray-400" aria-hidden="true" />
                {{ event.city }}
              </div>
              <div class="flex items-center mt-2 text-sm text-gray-500">
                <CalendarIconOutline class="flex-shrink-0 mr-1.5 h-5 w-5 text-gray-400" aria-hidden="true" />
                ...
                <div class="relative">
                  ...
                  <transition leave-active-class="transition duration-100 ease-in" leave-from-class="opacity-100"
                    ...
                    <ListboxOptions
                      ...
                      <ListboxOption as="template" v-for="action in actions" :key="action.name" :value="action"
                        ...
                        <li
                          ...
                          <component :is="action.icon" class="w-5 h-5 text-gray-500 dark:text-white-light"
                            aria-hidden="true" />
      ...
      <main class="pt-8 pb-16 rounded-b-lg dark:bg-white">
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
          <div class="px-4 sm:px-0">
            ...
            <!-- Tabs -->
            <TabGroup>
              <TabList class="flex items-center mt-2 -mb-px space-x-8 border-b border-gray-200" aria-label="Tabs">
                <Tab :disabled="employees.length === 0" v-slot="{ selected }">
                  <div :class="[selected ? 'border-purple-500 text-purple-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-200', 'whitespace-nowrap py-4 px-1 border-b-2 font-medium',
                        ...
                        <div class="flex items-center px-4 py-5 sm:py-6 sm:px-0">
                            ...
                            <div class="flex-1 min-w-0 px-4 md:grid md:grid-cols-2 md:gap-4">
                              <div>
                                ...
                                <p class="flex items-center mt-2 text-sm text-gray-500">
                                  <MailIconOutline class="flex-shrink-0 mr-1.5 h-5 w-5 text-gray-400"
                                    aria-hidden="true" />
                              ...
                              <div class="hidden md:block">
                                <div>
                                  ...
                                  <p class="flex items-center mt-2 text-sm text-gray-500">
                                    <CheckCircleIconOutline class="flex-shrink-0 mr-1.5 h-5 w-5 text-green-400"
                                      aria-hidden="true" />
                                    <!-- {{ employee.status }} -->
                                  </p>
                                </div>
                          ...
                          <div>
                            <button>
                              ...
                              <ChevronRightIconOutline class="w-5 h-5 text-gray-400 group-hover:text-gray-700"
                                aria-hidden="true" />
                            </button>
                          </div>
                        </div>
                    ...
                    <li v-for="file in files" :key="file.id" class="block group">
                      <div class="flex items-center px-4 py-5 sm:py-6 sm:px-0">
                        <div class="flex items-center flex-1 min-w-0">
                          <div class="flex-1 min-w-0 px-4 md:grid md:grid-cols-2 md:gap-4">
                            <div>
                              <p class="text-sm font-medium text-purple-600 truncate">{{ file.name }}</p>
                              <p class="flex items-center mt-2 text-sm text-gray-500">
                                <InformationCircleIconOutline class="flex-shrink-0 mr-1.5 h-5 w-5 text-gray-400"
                                  aria-hidden="true" />
